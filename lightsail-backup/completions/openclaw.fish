...
complete -c openclaw -n "__fish_use_subcommand" -a "sync" -d 'Gateway state sync helpers'
...
complete -c openclaw -n "__fish_seen_subcommand_from sync" -a "run" -d 'Run gateway sync once (or loop when --watch is set)'
complete -c openclaw -n "__fish_seen_subcommand_from run" -l watch -d 'Run sync on the configured interval'
