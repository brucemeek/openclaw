...
_openclaw_sync_run() {
  _arguments -C \
    "--watch[Run sync on the configured interval]"
}
...
_openclaw_sync() {
  ...
  _arguments -C \
     \
    "1: :_values 'command' 'run[Run gateway sync once (or loop when --watch is set)]'" \
    "*::arg:->args"

  case $state in
    (args)
      case $line[1] in
        (run) _openclaw_sync_run ;;
      esac
      ;;
  esac
}
